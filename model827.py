...
class Fusion_block(nn.Module):
    def __init__(self,in_ch):
        ...
        self.ca = ChannelAttention(in_ch,reduciton=2)

    def forward(self,ceus,bmodal):
        # ceus = ceus.mean(dim=2,keepdim=False)
        # print("ceus.shape", ceus.shape)
        # ceus = self.max(bmodal)
        # print("ceus.shape",ceus.shape)
        out = self.conv(bmodal)
        out = self.ca(out)
        # print(out.shape)
        # print(ceus.shape)
        out = torch.mul(torch.sigmoid(out).unsqueeze(2),ceus) + ceus
        return out
...
class ChannelAttention(nn.Module):
    def __init__(self,in_channels,reduciton=4):
        super().__init__()
        self.in_channels = in_channels
        self.reduction = reduciton

        self.fc1 = nn.Conv2d(in_channels,in_channels//reduciton,kernel_size=1,stride=1)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(in_channels//reduciton,in_channels,kernel_size=1,stride=1)
...
class CBAMBlock(nn.Module):
    def __init__(self,in_channels,reduction=16):
        super().__init__()
        self.channel_attentin = SEBlock(in_channels,reduction)
    ...
    def forward(self,x):
        x = self.channel_attentin(x)
        b,c,h,w = x.size()
        avg_out = torch.mean(x,dim=1,keepdim=True)
        max_out,_ = torch.max(x,dim=1,keepdim=True)
        concat = torch.cat([avg_out,max_out],dim=1)
        y = self.spatial_attention(concat)
        return x*y
...
class SelfAttentionClassifier(nn.Module):
    def __init__(self,inpit_dim,output_dim):
        super().__init__()
        self.attention = nn.MultiheadAttention(embed_dim=inpit_dim,num_heads=1)
        self.fc_out = nn.Linear(inpit_dim,output_dim)
...
class Resnet827(nn.Module):
    def __init__(self,in_ch,num_class,base_ch,num_frame,blocks_layers,attention_apply,fusion_apply,bmodal_apply):
        ...
        self.last_2d = nn.Sequential(
            nn.Conv2d(120,128,3,1,1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
        ...
        self.last_3d = nn.Sequential(
            nn.Conv3d(120,128,3,1,1),
            nn.BatchNorm3d(128),
            nn.ReLU(),
            # nn.Conv3d(base_ch, base_ch, 3, 1, 1),
            # nn.BatchNorm3d(base_ch),
            # nn.ReLU(),
            # nn.Conv3d(base_ch, base_ch, 3, 1, 1),
            # nn.BatchNorm3d(base_ch),
        ...
        self.adp2d = nn.AdaptiveAvgPool2d(1)
        self.adp3d = nn.AdaptiveAvgPool3d(1)
        self.fn1 = nn.Linear(128,num_class)
        self.fn = nn.Linear(128,num_class)
        self.out = nn.Linear(256,num_class)
        self.relu = nn.ReLU()
        self.out1 = nn.Linear(128,num_class)
        self.trans = SelfAttentionClassifier(480,2)
        self.transformer = Transformer(get_transformer_config(256))
        self.drop = nn.Dropout(p=0.5)
        # self.out1 = nn.Linear(120,num_class)
    ...
    def forward(self,x,bmodal):
        ...
        for i,layer in enumerate(self.layers):
            ...
            if i % 2 == 1:
                ...
                bmodal = bmodal_features[(i-1)//2]
                ceus = fusion(ceus,bmodal)
...
if __name__ == '__main__':
    ...
    bmodal = torch.randn((4,1,224,224))
